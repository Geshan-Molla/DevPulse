...
export const updateIssuesMiddleware = async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
        ...
        //check if contributor trying to update the status 
        if (singleUser.role === "contributor" && (req.body.status && (req.body?.status === "in_progress" || req.body?.status === "resolved"))) {
            return commonError(res, { status: 403, success: false, message: "Forbidden", error: "Contributors are not allowed to update the status of an issue" })
        }
